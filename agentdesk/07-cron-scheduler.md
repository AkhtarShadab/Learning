...
### Min-Heap — Cron Fire-Time Ordering

A min-heap is a complete binary tree where every parent node's key is smaller than or equal to its children's keys, giving O(log n) insertion and O(log n) extraction of the minimum element. The AgentDesk cron scheduler maintains a min-heap of scheduled jobs keyed by their `nextRunAt` timestamp — the root of the heap always holds the job that fires soonest, whether that's a 30-minute heartbeat for `learning-document-creator` or a weekly Friday summary for `master-agent`. When the scheduler's tick loop runs, it peeks at the root in O(1) to check if the next job is due; if so, it extracts it in O(log n), fires the job (sending the configured `message` to the target agent in an isolated session), computes the next fire time from the cron expression or interval, and re-inserts the job in O(log n). This is identical to how operating system timer queues and Linux's `timerfd` subsystem work under the hood — the timer wheel variant is used for coarse granularity, but the min-heap is the textbook approach for the kind of mixed-granularity scheduling AgentDesk performs (some jobs every 5 minutes, others monthly).

### Circular Buffer — Run History as Bounded Audit Trail

A circular buffer (or ring buffer) is a fixed-size array with head and tail pointers that wrap around modulo the capacity, giving O(1) append and O(1) eviction of the oldest entry without any memory reallocation. AgentDesk stores cron run records — the output of `ad-cron-runs <cronId>` showing each run's timestamp, success/failure status, duration, and summary — in a bounded history. When a job like the 30-minute heartbeat fires 48 times per day, the run history would grow without bound unless capped; the circular buffer approach keeps the most recent N runs (enough for meaningful debugging and auditing) while automatically evicting the oldest records as new ones arrive. This avoids unbounded storage growth while preserving the recent audit trail that the document identifies as "valuable for debugging and auditing." The same technique powers kernel ring buffers (`dmesg`), network packet capture buffers, and log rotation systems where bounded, overwrite-on-full semantics are the correct trade-off.

### Token Bucket — Rate Limiting Runaway Job Execution

A token bucket is a rate-limiting algorithm where tokens are added to a bucket at a fixed rate, and each operation consumes one token — if the bucket is empty, the operation is delayed or rejected. In the cron scheduler, a token bucket (or its equivalent, a leaky bucket) prevents runaway job execution: if a cron job's handler fails fast and the scheduler immediately re-queues it, or if a manual trigger via `ad-cron-run` is called repeatedly, the rate limiter ensures the agent isn't overwhelmed with concurrent sessions. Consider the heartbeat for `learning-document-creator` configured at `--every "30m"` — if a bug caused the handler to complete in 1 second, without rate limiting the scheduler could fire it again immediately, creating a tight loop of sessions. The token bucket caps the effective fire rate regardless of how quickly jobs complete, filling one token per configured interval and requiring one token per execution. This is the same mechanism that API gateways (nginx, AWS API Gateway) use to enforce request-per-second limits and that network traffic shapers use to smooth bursty transmission.

### Hash Map — Job Registry for O(1) Lookup by ID

A hash map maps keys to values via a hash function, providing average-case O(1) lookup, insertion, and deletion. The cron scheduler maintains a hash map of all registered jobs keyed by their unique `id` field, enabling every management command — `ad-cron <cronId>` to view details, `ad-cron-update <cronId>` to modify settings, `ad-cron-delete <cronId>` to remove, and `ad-cron-run <cronId>` to manually trigger — to resolve the target job in constant time. Without this index, locating a specific job among potentially dozens of registered crons (heartbeats for every agent, weekly reports, one-shot reminders, board sweeps) would require a linear scan. The hash map also enables the disable/enable toggle (`ad-cron-update <cronId> --enabled false`) to take effect immediately: the scheduler looks up the job by ID, flips the `disabled` boolean, and on the next tick the heap extraction skips disabled jobs. This is the same pattern used in process tables (PID-to-process lookup) and connection pools (socket-ID-to-connection lookup).

### State Machine — Cron Job Lifecycle States

A finite state machine is a model where an entity occupies exactly one of a finite set of states at any time, and transitions between states are triggered by defined events. Each cron job in AgentDesk follows a lifecycle state machine with states: Active (enabled, scheduled in the heap), Disabled (paused via `--enabled false`, removed from the scheduling heap but retained in the registry), Fired (currently executing an agent session), and Completed (for one-shot `--at` jobs that have fired and will not recur). The transitions are: Active to Fired on tick (when `nextRunAt` is reached), Fired back to Active on completion (after `nextRunAt` is recomputed), Active to Disabled on `ad-cron-update --enabled false`, Disabled to Active on re-enable, and Fired to Completed for one-shot jobs. The document notes that after firing, one-shot jobs "remain in the system for audit but don't fire again" — this is the absorbing state in the FSM, a state with no outgoing transitions. Understanding this lifecycle prevents confusion about why a one-shot job still appears in `ad-crons` output after it has executed.
