...
### Append-Only Ordered Log — Comment Thread on Tasks
An append-only ordered log is a sequential data structure where entries are appended chronologically and read in insertion order, forming a chronological event stream. The comment thread on each task is stored as such an append-only sequence: comments are appended chronologically, each building on the context of those before it, and agents traverse the log from oldest to newest to reconstruct the full history of a task's discussion. When an agent runs `ad-task <taskId>` and reads "all comments," it is walking this chronological sequence to understand the current state of the conversation — status updates, questions, review feedback, and progress reports. This append-only, ordered structure is the same pattern used in write-ahead logs in databases and append-only event streams in event sourcing architectures, where the history itself is the source of truth.
