...
A hash map stores key-value pairs with O(1) average-time lookup by hashing the key to a bucket index. The API server maintains a routing table that maps each (HTTP method, URL path pattern) pair to a handler function — when a request arrives for `POST /api/v1/projects/:id/tasks`, the router hashes the method+path combination and dispatches to the task-creation handler without scanning every registered route. With the endpoint catalogue listing 30+ routes across projects, tasks, subtasks, comments, contexts, crons, and agents, a linear scan for every incoming request would be wasteful. The hash-based routing table ensures that whether you hit `/stats` or `/projects/:id/tasks/:id/comments`, the dispatch cost is constant regardless of how many endpoints are registered. This is the same mechanism that Express.js, Fastify, and virtually every HTTP framework uses under the hood — the route table is fundamentally a hash map from pattern strings to handler functions.

### Request Queue — Managing Concurrent API Requests
A request queue is a FIFO data structure that buffers incoming work items and processes them in order, decoupling arrival rate from processing rate. When multiple agents fire their heartbeats simultaneously — say, three agents all calling `ad-tasks`, `ad-comment`, and `ad-status` within the same minute — the API server enqueues these requests rather than rejecting them or processing them chaotically. Each request enters the queue, awaits its turn for database access and computation, and returns a response. This is especially important for write operations: if two agents try to update the same task's status concurrently, the queue serializes these mutations and prevents race conditions like both agents believing they successfully moved a task to "in-progress." The Node.js event loop that AgentDesk runs on is itself a request queue — incoming HTTP connections are queued and processed one tick at a time, with I/O operations (database reads, file writes) handled asynchronously so the queue keeps draining.
...
A trie (prefix tree) is a tree structure where each node represents a character or path segment, and paths from root to leaf spell out complete keys, enabling O(k) lookup where k is the key length. REST API routers commonly use a radix trie (compressed trie) to match incoming URL paths against registered route patterns. Consider AgentDesk's nested routes: `/api/v1/projects/:id`, `/api/v1/projects/:id/tasks`, and `/api/v1/projects/:id/tasks/:id/comments` share a common prefix, and the trie structure avoids redundant prefix comparisons. When a request for `/api/v1/tasks/abc123/subtasks` arrives, the router walks the trie — `api` → `v1` → `tasks` → `:id` (captures `abc123`) → `subtasks` — and arrives at the handler in O(depth) time while extracting the `:id` parameter along the way. This is more efficient than regex-matching against every route pattern and is why frameworks like Express use path-to-regexp trees internally.
