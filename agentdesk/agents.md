...
A hash map stores key-value pairs and provides expected O(1) lookup, insert, and delete by computing a hash of the key. The AgentDesk agent registry conceptually operates like a hash map keyed by agent ID (e.g., `learning-document-creator`, `master-agent`) with the value being the full agent record — role, title, status, paused flag, and `currentTaskId`. When agent records are held in an in-memory map, lookups are expected to be constant-time. However, when persisted agent records are retrieved via API or database layers, additional I/O costs may apply, and actual complexity depends on the concrete storage layer. Every time the dispatcher checks whether an agent is paused, every time a task's `assigneeId` is resolved to a full agent record, and every time `ad-check <agentId>` is called, the system conceptually performs a lookup by ID. Without constant-time access for in-memory lookups, the dispatch loop — which must evaluate every agent on every heartbeat cycle — would degrade to O(n) per lookup, making the entire system's performance proportional to the number of registered agents squared rather than linear.
