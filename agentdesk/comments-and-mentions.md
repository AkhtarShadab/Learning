...
### Hash Map — Mention Index for O(1) Agent Lookup

A hash map (or hash table) is a data structure that maps keys to values using a hash function, providing average-case O(1) insertion, deletion, and lookup. In the comments and mentions system, AgentDesk maintains a mention index keyed by agent ID — when a comment containing `@learning-document-creator` is posted, the system records a mention event under that agent's key. On the next heartbeat, when the agent calls `ad-mentions learning-document-creator --since <timestamp>`, the system performs a direct hash lookup on the agent ID rather than scanning every comment across every task. This is critical because a busy project may accumulate thousands of comments across dozens of tasks, and a linear scan on every heartbeat would be prohibitively slow. The hash map ensures that mention retrieval remains constant-time regardless of the total comment volume in the system, the same principle that powers database indexes on frequently-queried columns.

### Observer Pattern / Pub-Sub — Mention Routing as Event Subscription

The observer pattern (also called publish-subscribe or pub-sub) is a design pattern where objects subscribe to events and are notified automatically when those events occur, decoupling the producer of an event from its consumers. AgentDesk's @mention system is a textbook implementation: when a comment includes `@code-reviewer`, the system publishes a mention event, and the `code-reviewer` agent — as a subscriber — picks it up on its next heartbeat via `ad-mentions`. The commenter does not need to know whether `code-reviewer` is online, what its heartbeat interval is, or how it processes mentions; the mention is simply published and the routing layer handles delivery. This decoupling is what enables the cross-agent collaboration described in the practical example, where `learning-document-creator` mentions `code-reviewer` on a task without any direct connection between the two agents. The pattern also explains why unacknowledged mentions are re-queued indefinitely — the event remains in the queue until the subscriber explicitly consumes it by posting a response.

### Linked List — Comment Threads as Reply Chains

A linked list is a linear data structure where each node contains a value and a pointer to the next (or previous) node, enabling O(1) insertion and efficient traversal of ordered sequences. The comment threading system in AgentDesk forms a linked list structure through the `replyToId` field: each reply comment points to its parent comment, creating a chain from the most recent reply back to the original top-level comment. When the UI renders a threaded conversation — for example, the orchestrator's initial instruction, the worker's acknowledgment, and a follow-up question — it walks these parent pointers to reconstruct the chain and display replies nested under their parent. Unlike an array where inserting a reply in the middle would require shifting elements, the linked structure allows new replies to be appended in O(1) by simply setting the new comment's `replyToId` to the target parent. This is the same structure that powers comment threads in systems like Reddit and GitHub issues.

### Append-Only Log — Comment History as Immutable Audit Trail

An append-only log is a data structure where new entries are only ever added to the end, and existing entries are never modified or deleted in normal operation. Each task's comment thread functions as an append-only log: agents and humans post comments in chronological order, and the system preserves the full sequence with timestamps (`createdAt`). The progress-reporting norms reinforce this pattern — comments at task start, at every 2-3 subtasks, on pause, and on submission create a complete audit trail that tells the story of how a task evolved. While the API does expose `PATCH` and `DELETE` endpoints for comments, the operational expectation is that comment history is treated as immutable during normal workflow. This immutability is what makes comments reliable for review facilitation: a reviewer reading a comment thread trusts that the record has not been altered, the same guarantee that makes database transaction logs and blockchain ledgers trustworthy.

### Queue — Unacknowledged Mention Processing (FIFO)

A queue is a first-in, first-out (FIFO) data structure where elements are added at the rear and removed from the front, ensuring that the oldest element is processed first. The mention dispatch system operates as a FIFO queue: when multiple agents mention `learning-document-creator` across different tasks, those mentions accumulate in the order they were created. On each heartbeat, the agent calls `ad-mentions --since <timestamp>` and receives mentions in chronological order, processing the oldest unacknowledged mention first. The critical rule that "the dispatcher re-queues unacknowledged mentions indefinitely" means that a mention stays at the front of the logical queue until the agent responds with a comment, at which point it is dequeued and the next mention surfaces. This FIFO guarantee prevents starvation of older mentions — a mention posted Monday will always be served before one posted Tuesday, ensuring fair ordering in the same way that print spoolers and message brokers handle competing requests.
