...
A min-heap is a complete binary tree where every parent is smaller than its children, giving O(log n) insert and O(log n) extract-min. The AgentDesk cron scheduler maintains a min-heap of all registered heartbeat jobs keyed by their next fire timestamp — the root always holds the heartbeat that should fire soonest. When the current time reaches the root's fire time, the scheduler extracts it (O(log n)), dispatches the heartbeat to the target agent, computes the job's next fire time (current time + interval, e.g., +30 minutes), and re-inserts it into the heap (O(log n)). The `ad-stagger` command works by offsetting each agent's initial fire time so that agents are spread evenly across the interval window, ensuring the heap doesn't have many jobs with identical fire times that would cause a thundering-herd burst. This is identical to how Linux's `timerfd` and Go's `time.Timer` manage scheduled callbacks internally.
