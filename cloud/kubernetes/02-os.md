...
```
 APARTMENT BUILDING (Operating System)
 ┌──────────────────────────────────────────────┐
 │  Apt 101 (Process A)    Apt 102 (Process B)  │
 │ ┌──────────────────┐   ┌──────────────────┐  │
 │ │ Roommate 1 (T1)  │   │ Roommate 1 (T1)  │  │
 │ │ Roommate 2 (T2)  │   │ Roommate 2 (T2)  │  │
 │ │                   │   │ Roommate 3 (T3)  │  │
 │ │ [shared kitchen]  │   │ [shared kitchen]  │  │
 │ │ [shared bathroom] │   │ [shared bathroom] │  │
 │ └──────────────────┘   └──────────────────┘  │
 │         WALLS BETWEEN APARTMENTS              │
 │     (separate address spaces, FDs, PIDs)      │
 └──────────────────────────────────────────────┘
```
...
```
  Process A's Map          Physical RAM           Process B's Map
 ┌──────────────┐        ┌──────────────┐       ┌──────────────┐
 │ 0x0000: code │───────▶│ Frame 42     │◀──────│ 0x0000: code │
 │ 0x1000: heap │───┐    │ Frame 43     │       │ 0x1000: heap │──┐
 │ 0x2000: stack│─┐ │    │ Frame 44     │       │ 0x2000: stack│  │
 └──────────────┘ │ │    │ Frame 45     │◀──┐   └──────────────┘  │
                  │ └───▶│ Frame 100    │   │                     │
                  └─────▶│ Frame 101    │   └─────────────────────┘
                         │ ...          │
...
```
  ...
  │  4  │ TCP socket to 10.0.0.5:80 │
  │  5  │ Unix socket to /tmp/db    │
  │  6  │ pipe (read end)           │
  └─────┴───────────────────────────┘
```
...
```
  Traditional polling (select):         epoll (event-driven):

  for each of 10,000 FDs:               Register 10,000 FDs once
    "anything ready?" ─── O(n)          Kernel maintains ready list
    "anything ready?"                    epoll_wait() returns ONLY
    "anything ready?"                      the FDs that are ready
    ...                                  ─── O(ready FDs), not O(total)
```
...
```
  PROCESS (single-threaded)          PROCESS (multi-threaded)
  ┌─────────────────────┐           ┌─────────────────────────────┐
  │  Code segment       │           │  Code segment (shared)      │
  │  Heap (data)        │           │  Heap (shared)              │
  │  File descriptors   │           │  File descriptors (shared)  │
  │  ┌───────────────┐  │           │  ┌────────┐ ┌────────┐     │
  │  │ Stack (main)  │  │           │  │Stack T1│ │Stack T2│ ... │
  │  │ Registers     │  │           │  │Regs T1 │ │Regs T2 │     │
  │  └───────────────┘  │           │  └────────┘ └────────┘     │
  └─────────────────────┘           └─────────────────────────────┘
```
...
```
  Virtual Address Space (per process)     Physical RAM (shared)
  ┌──────────────────┐                   ┌──────────────────┐
  │ Page 0  ─────────────────────────────▶ Frame 7          │
  │ Page 1  ─────────────────────────────▶ Frame 42         │
  │ Page 2  ── (not in RAM, on disk) ──▶ │ Frame 43         │
  │ Page 3  ─────────────────────────────▶ Frame 100        │
  │ Page 4  ── (not allocated) ────────▶ │ ...              │
  │ ...     │                             │                  │
  └──────────────────┘                   └──────────────────┘
                  │
          PAGE TABLE (per process)
          translates virtual page → physical frame
          also stores: present bit, dirty bit, access bit
```
...
```
  ps aux                    pipe (kernel buffer)              grep nginx
  ┌──────┐  stdout (FD 1)  ┌──────────────────┐  stdin (FD 0)  ┌──────┐
  │ write ├────────────────▶│  4 KB buffer     ├───────────────▶│ read │
  └──────┘                  └──────────────────┘                └──────┘
```
...
```
  ...
  ┌─────────────────┬───────────┬────────────┬──────────────┐
  │ Mechanism       │ Speed     │ Structured │ Scope        │
  ├─────────────────┼───────────┼────────────┼──────────────┤
  │ Signals         │ Fast      │ No data    │ Same host    │
  │ Pipes           │ Medium    │ Byte stream│ Parent-child │
  │ Named pipes     │ Medium    │ Byte stream│ Same host    │
  │ Unix sockets    │ Fast      │ Stream/Dgram│ Same host   │
  │ TCP sockets     │ Slower    │ Byte stream│ Any network  │
  │ Shared memory   │ Fastest   │ Raw bytes  │ Same host    │
  │ Message queues  │ Medium    │ Messages   │ Same host    │
  └─────────────────┴───────────┴────────────┴──────────────┘
```
...
```
  ...
  │ Interface    │ Purpose                                     │
  ├──────────────┼─────────────────────────────────────────────┤
  │ lo           │ Loopback (127.0.0.1) — talk to yourself    │
  │ eth0 / ens*  │ Physical ethernet NIC                       │
  │ docker0      │ Docker bridge — containers connect here     │
  │ cni0 / cbr0  │ Kubernetes CNI bridge                       │
  │ veth*        │ Virtual ethernet pair (container ↔ bridge)  │
...
```
  Packet routing decision:
                    ┌──────────────────┐
  Packet arrives    │ Destination IP?  │
  ─────────────────▶│                  │
                    └────────┬─────────┘
                             │
              ┌──────────────┼──────────────┐
              │              │              │
         10.0.0.0/24   172.17.0.0/16   everything else
              │              │              │
              ▼              ▼              ▼
           eth0          docker0       default gw
                                      (10.0.0.1)
```
...
```
  ...
  ├── /sbin                  ← Essential system binaries (iptables, mount)
  ├── /etc                   ← System-wide configuration files
  │   ├── /etc/hosts         ← Static DNS entries
  │   ├── /etc/resolv.conf   ← DNS nameserver config
  │   ├── /etc/fstab         ← Filesystem mount table
  │   ├── /etc/passwd        ← User account info
  │   └── /etc/systemd/      ← Systemd unit files
  ├── /proc                  ← Virtual filesystem — live kernel/process data
  │   ├── /proc/<pid>/       ← Per-process info (status, maps, fd, ns)
  │   ├── /proc/meminfo      ← System memory statistics
  │   ├── /proc/cpuinfo      ← CPU information
  │   └── /proc/sys/         ← Tunable kernel parameters
  ├── /sys                   ← Virtual filesystem — device/driver/kernel objects
  │   └── /sys/fs/cgroup/    ← Cgroup hierarchy (resource limits)
  ├── /dev                   ← Device files
  ...
  │   ├── /var/lib/docker/   ← Docker storage (images, containers, volumes)
...
```
  -rwxr-xr--  1  shadab  developers  4096  May 21 10:00  script.sh
  │├─┤├─┤├─┤     │       │
  │ │  │  │      owner   group
  │ │  │  │
  │ │  │  └── Others: r-- (read only)
...
```
  CFS Red-Black Tree (sorted by vruntime):
        ...
        ┌──────┴──────┐     ┌─────┴──────┐
   ┌────┴────┐  ┌─────┴───┐│ Task D     │
   │ Task B  │  │ (empty) ││ vrt=600    │
   │ vrt=200 │  └─────────┘└────────────┘
   └─────────┘
   ↑ Leftmost node = smallest vruntime = NEXT to schedule
```
...
```
  ...
  │         ╱    ╲                                   │
  │     ┌────┐  ┌─────┐                             │
  │     │fd=3│  │fd=12│   ← O(log n) add/remove    │
  │     └────┘  └─────┘                             │
  │                                                  │
  │  Ready List (FDs with pending events)            │
  │  ┌────┐ → ┌────┐ → ┌─────┐                     │
  │  │fd=3│   │fd=7│   │fd=12│  ← O(1) retrieval   │
  │  └────┘   └────┘   └─────┘                     │
  └─────────────────────────────────────────────────┘
```
...
```
  Path lookup: /var/log/nginx/access.log

  dentry cache (hash table):
  ┌────────────┬──────────┬─────────────┐
  │ Key (hash) │ Dentry   │ Inode ptr   │
  ├────────────┼──────────┼─────────────┤
  │ hash("/")  │ /        │ inode 2     │
  │ hash("var")│ var      │ inode 1001  │
  │ hash("log")│ log      │ inode 1050  │
  │ hash(...)  │ nginx    │ inode 2003  │
  │ hash(...)  │ access.. │ inode 2047  │
  └────────────┴──────────┴─────────────┘

  LRU list (least recently used evicted first):
  HEAD ← access.log ← nginx ← log ← var ← / → TAIL
...
```
  ...
  │ (dev, ino)       │                                      │
  ├──────────────────┼──────────────────────────────────────┤
  │ (sda1, 2)        │ type=dir, perms=755, size=4096,     │
  │                  │ uid=0, gid=0, blocks=[...], links=23│
  ├──────────────────┼──────────────────────────────────────┤
  │ (sda1, 2047)     │ type=file, perms=644, size=1048576, │
  │                  │ uid=33, gid=33, blocks=[...], links=1│
  ├──────────────────┼──────────────────────────────────────┤
  │ (sda2, 100)      │ type=file, perms=600, size=256,     │
  │                  │ uid=0, gid=0, blocks=[...], links=1 │
  └──────────────────┴──────────────────────────────────────┘
```
