...
```
Type 1 ("The Foundation"):             Type 2 ("Room Within a Room"):
 ...
 │  (bare metal — no host OS)│          │  (runs as an app)   │
 └───────────┬───────────────┘          └─────────┬──────────┘
             │                                    │
 ┌───────────┴───────────────┐          ┌─────────┴──────────┐
...
```
Hardware Abstraction (VMs):              OS Abstraction (Containers):
...
Each VM: full OS kernel                 All containers: shared kernel
Strong isolation boundary               Lighter, faster, less isolation
```
...
```
Performance Stack (simplified):

Type 1:                              Type 2:
┌────────────┐                       ┌────────────┐
│ Guest Code │ ←─ runs at            │ Guest Code │ ←─ runs at
│            │    near-native        │            │    near-native
│            │    speed (VT-x)       │            │    speed (VT-x)
├────────────┤                       ├────────────┤
│ Hypervisor │ ←─ thin layer,        │ Hypervisor │ ←─ must go through
│ (bare metal)│   direct HW access   │  (app)     │    host OS for I/O
├────────────┤                       ├────────────┤
...
│  │  └──────────┘  └──────────┘  └──────┘  │   │
│  │                                         │   │
│  │  ioctl(/dev/kvm) for CPU execution      │   │
│  └───────────────────┬────────────────────┘   │
│                      │                         │
├──────────────────────┼─────────────────────────┤
│                Kernel Space                    │
...
│  ┌────┴──────────────┴────┐                     │
│  │  Virtual NAT Router     │                     │
│  │  (virbr0: 192.168.122.1)│  ← dnsmasq DHCP   │
│  └────────────┬────────────┘                     │
│               │ NAT                              │
│  ┌────────────┴────────────┐                     │
│  │  Host Network Stack     │                     │
...
```
Circular Buffer (virtio vring):

  Available Ring (guest → host):
  ┌───┬───┬───┬───┬───┬───┬───┬───┐
  │ 3 │ 7 │ 1 │ 5 │   │   │   │   │  ← guest writes descriptor indices
  └───┴───┴───┴───┴───┴───┴───┴───┘
    ↑                   ↑
    host reads here     guest writes here
    (consumer index)    (producer index)

  Used Ring (host → guest):
  ┌───┬───┬───┬───┬───┬───┬───┬───┐
  │ 3 │ 7 │   │   │   │   │   │   │  ← host writes completed descriptors
  └───┴───┴───┴───┴───┴───┴───┴───┘
    ↑       ↑
    guest   host writes here
    reads
```
...
Bit:  [0][1][0][0][1][0][0][0][1][0][ 0][ 0][ 1]...
       │  ↑        ↑              ↑                ↑
       │  dirty    dirty          dirty            dirty
       clean

→ Copy pages 1, 4, 8, 12, ... to destination
→ Clear all bits to 0
→ Repeat until dirty count is small enough for final stop-and-copy
```
