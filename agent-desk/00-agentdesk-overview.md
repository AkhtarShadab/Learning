...
A **priority queue** is an abstract data type where each element has a priority, and the element with the highest priority (lowest numeric value in a min-heap) is always extracted first. AgentDesk tasks have a `priority` field (0 = critical, 1 = high, 2 = medium, 3 = low), and the dispatcher always selects the lowest-priority-number task for an agent to work on. This is conceptually analogous to a min-heap extraction: each dispatch tick can be thought of as an `extractMin()` over the agent's eligible task set, though the actual implementation may differ. The agent's own rules reinforce this — rejected tasks (which already have context loaded) take precedence, then recently resumed tasks, then the highest-priority assigned task, with creation timestamp as a tiebreaker. Without priority-queue semantics, the dispatcher would need an O(n) scan of all tasks every tick; a heap-ordered structure would enable O(log n) selection, though actual runtime behavior depends on the specific implementation.

### N-ary Tree — Project → Task → Subtask Hierarchy

An **N-ary tree** is a rooted tree where each node can have an arbitrary number of children. AgentDesk's data model forms a three-level N-ary tree: `Project` (root) → `Task` (children) → `Subtask` (leaves). A project contains many tasks (each with its own status, assignee, and priority), and each task can contain many subtasks (lightweight checklist items). This tree structure is reflected directly in the database schema — `tasks.projectId` is the parent pointer from task to project, and `subtasks.taskId` is the parent pointer from subtask to task. Tree traversal analogies are instructive: when a project is paused, the system propagates that pause downward to all tasks (similar to a pre-order traversal pattern). When checking if a task is "complete," the system checks all leaf subtasks (similar to a post-order check — all children must be `done` before the parent can move to `review`), though these traversal patterns are conceptual illustrations rather than guaranteed runtime behavior.
