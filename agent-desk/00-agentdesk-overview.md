...
### Priority Queue — Dispatcher Work Ordering

A **priority queue** is a data structure that always surfaces the highest-priority element in O(log n) time, typically backed by a binary heap. In the context of AgentDesk's dispatcher (`dispatcher.ts`), each eligible agent's work queue is implicitly prioritized: rejected tasks take precedence over resumed tasks, which take precedence over new assignments, which are further sorted by the `priority` column (0=critical, 1=high, 2=medium, 3=low). When a heartbeat tick fires, the dispatcher effectively performs an extract-min operation across these work items to build the heartbeat prompt with the most urgent work listed first. Implementing this explicitly as a min-heap keyed on `(rejection_flag, priority, created_at)` would formalize the O(log n) insertion and extraction that the current SQL `ORDER BY` achieves per query.

### Finite State Machine — Task Lifecycle

A **finite state machine (FSM)** is a computational model with a fixed set of states, a set of transitions between those states, and rules governing when each transition is valid. The task lifecycle in AgentDesk (Section 6) is a textbook FSM: states are `todo → assigned → planning → in-progress → review → done/rejected`, with `paused` as an orthogonal flag on any state. Each transition is triggered by a specific API call (`ad-status`, `ad-submit`, approve, reject), and invalid transitions are rejected by the `PATCH /tasks/:id/status` handler. This FSM design prevents illegal state jumps (e.g., moving directly from `todo` to `done`) and makes the system's behavior deterministic and auditable — every state change is recorded in the `activities` audit log.

### FIFO Queue with Mutex — Per-Agent Session Locking

A **FIFO queue** (first-in, first-out) processes elements in the order they arrive, and a **mutex** (mutual exclusion lock) ensures only one consumer operates on a shared resource at a time. The SessionPool's `withAgentLock()` combines both: it chains promises so that concurrent `sendTurn()` calls for the same agent are serialized in arrival order, while turns for different agents execute in parallel with no contention. This is equivalent to maintaining a per-key FIFO queue where the key is the agent ID, and the dequeue operation is the resolution of the preceding promise. Without this structure, concurrent writes to the same JSONL session file would interleave messages, corrupting the conversation history — the same class of bug that database write-ahead logs and disk schedulers use queuing to prevent.

### Hash Map — Provider Routing Table

A **hash map** provides O(1) average-case lookup by key, making it ideal for routing tables where a request must be dispatched to the correct handler quickly. AgentDesk's LLM proxy uses what is effectively a hash map keyed on provider ID: when an SDK subprocess sends a request with an HMAC-signed routing key (`agdesk-proxy-<providerId>.<hmac>`), the proxy handler extracts the provider ID, verifies the HMAC in O(1), and looks up the provider's encrypted credentials and base URL from the `providers` table (cached in memory after first access). This pattern mirrors how network routers use hash-based forwarding tables to direct packets — each incoming request is routed to the correct upstream LLM endpoint without scanning the full provider list.

*Document generated by master-agent on 2026-04-24. Based on `@zish/agent-desk` v0.1.1 source at `/home/shadab/.nvm/versions/node/v24.13.0/lib/node_modules/@zish/agent-desk/`.*
