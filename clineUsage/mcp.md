...
### Adapter Pattern — Protocol Translation Between Client and Server

The **adapter pattern** wraps an interface that a client cannot use directly and translates it into one the client expects, enabling integration without modifying either side. MCP's entire architecture is a three-layer adapter stack: the LLM expects a uniform tool-call interface (name, typed JSON parameters, structured result), but each external service speaks its own protocol — GitHub speaks REST with OAuth headers, Postgres speaks the wire protocol with SQL, Slack speaks a WebSocket-based Events API. Each MCP server is a concrete adapter that accepts the LLM's standardized JSON-RPC tool call, translates it into the native service protocol (HTTP request, SQL query, WebSocket message), executes the operation, and translates the response back into a structured MCP content object. This is why the document repeatedly emphasizes that MCP tools "appear alongside built-ins" — the adapter makes a `create_github_issue` call indistinguishable from a native `search` call from the LLM's perspective. Without this adaptation layer, every new external service would require changes to Cline's core tool dispatch loop, violating the open-closed principle that makes the ecosystem scalable.

### Connection Pool (Object Pool) — MCP Client Managing Server Connections

An **object pool** maintains a set of initialized, reusable objects to avoid the overhead of repeated creation and destruction, providing O(1) checkout and return operations. Cline's MCP client layer manages connections to multiple MCP servers simultaneously — the GitHub server, Postgres server, Brave Search server, and any custom servers all run as persistent child processes or HTTP/SSE connections. Rather than launching and tearing down a server process for each tool call, the MCP client maintains a pool of live connections that persist across the entire session, reusing the same stdio pipe or SSE channel for every request to that server. The remote HTTP/SSE transport pattern described in the document — where "each SSE connection gets its own MCP server instance" — is the server-side complement: the Express app pools incoming connections and routes messages to the correct server instance. This pooling is what makes chaining MCP tools across servers efficient — the five-step incident response workflow (Slack read, Postgres query, Brave search, GitHub issue creation, Slack post) executes against five already-warm connections rather than paying five cold-start penalties.

### Hash Map — Tool Registry and Server Discovery

A **hash map** provides O(1) average-time key-value lookup by hashing a key to an array index. The MCP configuration file is itself a hash map: the top-level JSON object maps server names (`"github"`, `"postgres"`, `"brave-search"`) to their connection configurations (command, args, env, or url and headers), enabling instant lookup of any server's details by name. When an MCP server connects, it advertises its tools via the `tools/list` handshake, and Cline inserts each tool into a unified tool registry — a hash map keyed by fully qualified tool names (e.g., `"github.create_issue"`, `"postgres.query"`) with values containing the tool's description, input schema, and a reference to the owning server's connection. When the LLM emits a tool call, the runtime hashes the tool name in O(1) to locate both the handler metadata and the correct server connection to route the request to. The security model also leverages a hash map: the `autoApprove` configuration maps tool names to approval policies, allowing O(1) permission checks on every invocation without scanning a list.

### JSON Schema Validation as a Type System (Structural Typing)

**Structural typing** determines type compatibility by examining the shape (structure) of data rather than its declared name, and JSON Schema is the runtime equivalent: it validates that a value conforms to a declared structure of properties, types, and constraints. Every MCP tool declares an `inputSchema` as a JSON Schema object — the `create_github_issue` tool specifies that `owner` is a required string, `labels` is an optional array of strings, and so on. When the LLM constructs a tool call, the MCP runtime validates the arguments against this schema before forwarding the request to the server, catching type errors (wrong type, missing required field, extra properties) at the protocol boundary rather than inside the server's handler. This is functionally equivalent to how a compiler's type checker validates function arguments against parameter declarations: the schema is the function signature, the tool call arguments are the actual parameters, and validation is the type-checking pass. The document's comparison of MCP versus raw terminal commands highlights this: `bash` commands have no schema (the LLM must guess the correct flag syntax), while MCP tools have explicit structural contracts that make malformed calls impossible.
