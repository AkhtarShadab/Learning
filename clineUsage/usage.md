...
### State Machine — Task Lifecycle Transitions on the Kanban Board

A **finite state machine** (FSM) defines a system as a set of discrete states with explicit transitions triggered by events, where the system is always in exactly one state at a time. The Kanban board's column flow — `ASSIGNED → PLANNING → IN-PROGRESS → REVIEW → DONE` with the `REJECTED → REVIEW` loop — is a six-state FSM where each transition is triggered by an `ad-status` or `ad-submit` command. The transition rules are strict: a task in `ASSIGNED` can only move to `PLANNING` or `IN-PROGRESS`, never directly to `DONE`; a task in `REVIEW` can only transition to `DONE` (approved by human) or `REJECTED` (sent back for rework). The `ad-pause` command adds a suspended meta-state that removes the task from the dispatcher's active set — the FSM pauses until a human triggers the resume transition. This formalization is why the document emphasizes "pausing is the ONLY way to stop the dispatcher from re-triggering you" — without the explicit FSM transition to the paused state, the system remains in `IN-PROGRESS` and the dispatcher's tick loop continues scheduling work on it.

### Priority Queue — Task Scheduling and "What Should I Work On Next?"

A **priority queue** is a data structure that always surfaces the highest-priority element in O(log n) time, typically backed by a binary min-heap. The "What should I work on next?" pattern — `ad-tasks <projectId> --assignee <myId> --status assigned`, then pick the highest priority (lowest number) — is conceptually extracting the minimum element from a priority queue keyed by the task's priority field (0 = critical, 1 = high, 2 = medium, 3 = low). In a production AgentDesk deployment with dozens of tasks across multiple projects, the dispatcher maintains this priority ordering so that when an agent's heartbeat fires, it does not scan all tasks linearly but rather checks the top of the priority queue. The "rejected > resumed > highest priority > oldest" tiebreaking rule described in the CLAUDE.md heartbeat workflow is a composite comparator: first sort by status bucket (rejected tasks are most urgent), then by priority level, then by creation timestamp — a multi-key comparison function passed to the heap's insertion and extraction operations.

### DAG (Directed Acyclic Graph) — Subtask Dependencies and Execution Ordering

A **directed acyclic graph** (DAG) models dependencies where nodes have directed edges but no cycles, enabling topological sorting to determine a valid execution sequence. The "Structured Multi-Step Work" pattern — `ad-plan <id> <agent> "Design schema" "Build endpoint" "Write tests" "Document"` — creates a linear DAG where each subtask depends on its predecessor: you cannot write tests before the endpoint exists, and you cannot document what has not been built. The `ad-subtask-done` command marks a node as completed, enabling the next node in topological order. More complex tasks create branching DAGs: "Design schema" might fan out to both "Build endpoint A" and "Build endpoint B" (independent, parallelizable), which then converge on "Write integration tests" (depends on both). The Kanban board's dependency chains enforce this DAG structure — a card blocked by uncompleted dependencies cannot enter `IN-PROGRESS`, which is the topological sort's guarantee that no node executes before all its predecessors are satisfied.

### Circular Buffer — WORKING.md as a Bounded Session Cache

A **circular buffer** (ring buffer) is a fixed-size array that wraps around, overwriting the oldest entry when full, providing O(1) append and bounded memory usage. The `WORKING.md` file functions as a bounded session cache: agents write their current focus, subtask progress, notes, and "Last Checked" timestamps before stopping, and on the next heartbeat they read this file to resume context. Because WORKING.md must remain concise (the document advises updating it "before stopping" with only the essential state), it behaves like a circular buffer where new session state overwrites old session state — the "TASK abc123" entry is replaced with updated subtask checkboxes and notes, not appended indefinitely. The "Last Checked" timestamps serve as the buffer's read pointer: the agent knows which tasks have been inspected since the last heartbeat and which need re-checking, preventing redundant work. If WORKING.md grew unboundedly across hundreds of heartbeats, it would bloat the agent's context window — the bounded, overwrite-on-update design keeps it a fixed-cost context load on every session start.
