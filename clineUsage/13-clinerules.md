...
### Trie — Conditional Rule Path Matching and Glob Resolution

A **trie** (prefix tree) is a tree data structure where each node represents a character (or in the case of file paths, a path segment), enabling O(k) prefix-based search where k is the key length. The conditional rules system uses glob patterns like `src/components/**` and `*.test.ts` to determine which rule files load for a given task. Internally, resolving these globs against the workspace file tree is a trie walk: the workspace directory structure is itself a trie where each node is a directory and each leaf is a file, and the glob pattern `src/components/**` is a prefix query that matches all leaves under the `src → components` branch. When Cline begins a task touching `src/components/Button.tsx`, it walks the path trie to check which conditional rules have matching patterns — the `components.md` rule matches (its pattern `src/components/**` is a prefix of the file path), while `database.md` (pattern `prisma/**`) does not. This is why conditional rules prevent context bloat: the trie walk prunes non-matching rule branches in O(k) per file path, avoiding a linear scan of all rule files against all active files.

### Hash Map — Rule Layer Resolution and Conflict Override

A **hash map** provides O(1) average-time key-value lookup by hashing a key to an array index. The two-layer rule system — global rules at `~/Documents/Cline/Rules/` and workspace rules at `.clinerules/` — is resolved via a hash map merge: each rule file is keyed by its filename (e.g., `code-style.md`), and the workspace map is overlaid on the global map. When a filename exists in both locations, the workspace value overwrites the global value in O(1) — this is the "workspace rules take precedence" behavior described in the document. The auto-detection of multiple rule formats (`.clinerules`, `.cursorrules`, `.windsurfrules`, `AGENTS.md`) is also a hash map lookup: Cline checks a fixed set of known filenames against the project root's directory listing, where each filename key maps to a parser/loader function. The `/newrule` slash command performs a hash map insert: it creates a new entry in the `.clinerules` directory with the rule content as the value, instantly available on the next task's rule-loading pass.

### Bloom Filter / Set Membership — Forbidden Pattern Detection

A **Bloom filter** is a probabilistic set-membership data structure that can definitively say "not in the set" in O(1) and "probably in the set" with a configurable false-positive rate, using far less memory than storing the full set. While Cline does not literally use a Bloom filter, the "Forbidden Patterns" and "Banned Patterns" sections in `.clinerules` function as a conceptual set-membership test applied to every code change: before approving a write, the LLM checks the proposed code against the banned-pattern set (`new PrismaClient()`, `useEffect` for data fetching, bare `except:` clauses, `.then()` chains). Each forbidden pattern is a member of the "reject set," and every code edit is tested for membership. The "Lessons Learned" section extends this to a historical reject set: each encoded bug becomes a new member (`SELECT ... FOR UPDATE` required for cart modifications, import paths must match file casing exactly). The key insight is that this set-membership check runs on every tool call — checking "does this edit contain a forbidden pattern?" is the same O(1)-per-pattern operation that makes Bloom filters efficient for filtering large streams, and it is why the document advises keeping rules specific and actionable: vague patterns ("write clean code") cannot be tested for membership, while concrete patterns (`state.items.push()` is banned) can.

### Layered Cache — Rule Loading Priority and Progressive Context Assembly

A **layered cache** (multi-level cache) checks successive levels for a value, returning the first hit: L1 cache (smallest, fastest) is checked before L2 (larger, slower), which is checked before main memory. The `.clinerules` loading system implements a three-level cache for project context: Level 1 is the unconditional always-loaded rules (stack.md, architecture.md, code-style.md) — small, always-resident, checked first. Level 2 is the conditional path-scoped rules (components.md, testing.md, database.md) — loaded only when the current file path matches their glob patterns, like an L2 cache that activates on specific access patterns. Level 3 is the linked external documentation (`docs/API_SPEC.md`, `docs/BILLING_FLOWS.md`) referenced by the "Before You Touch These Areas, Read First" table — loaded on-demand only when the agent decides it needs deeper context, like a cache miss that falls through to main memory. This layered approach is why the document advises keeping each rule file under 5K tokens: small L1 entries ensure the always-loaded context budget is predictable, while L2 and L3 entries scale without penalizing unrelated tasks. The total context cost is proportional to the working set, not the total rule set — exactly the property that makes multi-level caches efficient.
